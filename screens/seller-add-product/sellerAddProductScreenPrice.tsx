...
const SellerAddProductScreenPrice = ({ route,navigation }) => {
    const { t, i18n } = useTranslation();
    const { step_one, step_two, totalQuantity } = route.params;
    ...
    useEffect(() => { 
        ...
        AsyncStorage.getItem('step_three').then((step_three) => {
            if(step_three){
    ...
    const onSubmit = async (data) => {
        ...
        try {                  
            ...
            setLoading(false);
            navigation.navigate('SellerAddProductImages', { step_one: step_one, step_two: step_two, step_three: payload, totalQuantity: totalQuantity }); 
        } catch (e) {
          setLoading(false); 
          console.log(e)       
        }
                        ...
                        {inputs.map((input, key)=>(
                            <View style={styles.inputContainer}>
                                <View style={styles.viewContainer}>
                                  <TextInput placeholder={"Min " + selectedUnitType} value={input.value} onChangeText={(text)=>addMinHandler(text,key)} style={styles.textInputStyle}/>
                                  <TextInput placeholder={"Max " + selectedUnitType} value={input.value} onChangeText={(text)=>addMaxHandler(text,key)} style={styles.textInputStyle}/>
                                </View>
                                <View style={styles.viewContainer}>
                                  <TextInput placeholder={"Value (INR)"} value={input.value} onChangeText={(text)=>addPriceHandler(text,key)} style={styles.textInputStyle}/>
...
export default SellerAddProductScreenPrice;